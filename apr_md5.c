/*
 * Copied from the Apache Library and hacked together to be a self-standing
 * source file by Clifford Wolf <clifford@clifford.at>.
 */
...
/*** copied from httpd-2.0.49/srclib/apr/strings/apr_cpystrn.c ***/
...
/*** copied from httpd-2.0.49/srclib/apr-util/include/apr_md5.h ***/
                                  ...
                                  size_t inputLen)
{
    ...
    if ((rv = apr_md5_update(&ctx, input, inputLen)) != 0)
        return rv;
...
int apr_md5_encode(const char *pw, const char *salt,
 ...
 * of the smashed input password.  Return 0 if they match, or
 * -1 if they don't.
 */

int apr_password_validate(const char *passwd, 
                                                const char *hash)
{
    char sample[120];
#if !defined(WIN32) && !defined(BEOS) && !defined(NETWARE)
...
#error Configuration error!  crypt_r() should have been selected!
    ...
    return (strcmp(sample, hash) == 0) ? 0 : -1;
}
